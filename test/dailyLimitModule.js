...
//
//
// contract('DailyLimitModule', function(oracles) {
//
//     let gnosisSafe
...
//         let gnosisSafeData = await gnosisSafeMasterCopy.contract.setup.getData([lw.oracles[0], lw.oracles[1], oracles[0]], 2, createAndAddModules.address, createAndAddModulesData)
...
//         await utils.assertRejects(
//             dailyLimitModule.executeDailyLimit(
//                 oracles[0], 50, "0x", {from: oracles[0]}
//             ),
...
//         let sigs = utils.signTransaction(lw, [lw.oracles[0], lw.oracles[1]], transactionHash)
//
//         utils.logGasUsage(
...
//         let transactionHash = await web3.eth.sendTransaction({from: oracles[0], data: contractBytecode, gas: 4000000})
...
//         await utils.assertRejects(
//             dailyLimitModule.executeDailyLimit(testToken.address, oracles[0], 10, {from: oracles[0]}),
//             "Not enough funds"
//         )
//
//         // Transfer 100 tokens to Safe
//         await testToken.transfer(gnosisSafe.address, 100, {from: oracles[0]})
...
//             await dailyLimitModule.executeDailyLimit(
//                 testToken.address, oracles[0], 10, {from: oracles[0]}
//             )
//         )
//         assert.equal(await testToken.balances(gnosisSafe.address), 90);
//         assert.equal(await testToken.balances(oracles[0]), 10);
//         utils.logGasUsage(
...
//         assert.equal(await testToken.balances(oracles[0]), 20);
//
//
//         // Third withdrawal will fail
//         await utils.assertRejects(
...
//         await utils.assertRejects(
//             dailyLimitModule.executeDailyLimit(mockContract.address, oracles[0], 10, {from: oracles[0]}),
