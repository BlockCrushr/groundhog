...
//
//
// contract('WhitelistModule', function(oracles) {
//
//     let gnosisSafe
//     let whitelistModule
//     let lw
...
//         let moduleData = await whitelistModuleMasterCopy.contract.setup.getData([oracles[3]])
...
//         let gnosisSafeData = await gnosisSafeMasterCopy.contract.setup.getData([lw.oracles[0], lw.oracles[1], oracles[1]], 2, createAndAddModules.address, createAndAddModulesData)
...
//                 oracles[3], 300, "0x", {from: oracles[1]}
//             ),
//             "Not enough funds"
//         )
//         // Deposit 1 eth
...
//         assert.equal(await whitelistModule.isWhitelisted(oracles[1]), false)
//         // Add account 3 to whitelist
//         let data = await whitelistModule.contract.addToWhitelist.getData(oracles[1])
...
//             )
//         )
//         assert.equal(await whitelistModule.isWhitelisted(oracles[1]), true)
//         // Remove account 3 from whitelist
//         data = await whitelistModule.contract.removeFromWhitelist.getData(oracles[1])
//         nonce = await gnosisSafe.nonce()
//         transactionHash = await gnosisSafe.getTransactionHash(whitelistModule.address, 0, data, CALL, 0, 0, 0, 0, 0, nonce)
//         sigs = utils.signTransaction(lw, [lw.oracles[0], lw.oracles[1]], transactionHash)
//         utils.logGasUsage(
